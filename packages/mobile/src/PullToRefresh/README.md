...
- 默认检测最近可滚动祖先是否位于顶部，并在锁定纵向手势前复核最新 scrollTop；复杂宿主可通过平台中立的 `canPull()` 覆盖边界判断。
- 状态依次为 `idle / pulling / ready / refreshing / complete`。只有越过 `threshold` 后松手才调用一次 `onRefresh`。
- `onRefresh` 可以同步返回或返回 Promise；外部 Promise pending 即 refreshing 的唯一事实源并自动阻止并发，完成后展示 complete，拒绝时调用 `onRefreshError`、复位并允许重试。
...
- `touchcancel`、`pointercancel`、多指和禁用切换均安全复位；React 19 callback ref 的 cleanup 会原样执行。
