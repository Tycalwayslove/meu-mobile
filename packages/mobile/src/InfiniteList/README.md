...
- 默认在尾部进入最近声明 `overflow-y:auto/scroll` 的滚动祖先底部 250px 范围时触发；即使空列表/首屏尚未溢出也会绑定该容器，`threshold` 只接受非负有限数。
- `autoLoad={false}` 可切换为纯手动加载；IntersectionObserver 缺失或宿主初始化失败时自动保留原生“加载更多”按钮作为降级。
...
- 系统 reduced-motion 与 `ConfigProvider motion="reduced"` 都会停止 spinner；长状态/按钮文案允许在窄容器内换行，RTL 与 Dark 由 ConfigProvider Token 边界继承。
