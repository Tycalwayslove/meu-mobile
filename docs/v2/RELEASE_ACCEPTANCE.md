...
| 门禁         | 当前证据                                                                                                                                                                                               |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 文档映射     | 68 个产品、125 个公开值、384 个公开类型、68 份共置文档；44/68 已完成本地验证，24 个仍有本地缺口；0 未认领、0 覆盖问题                                                                                  |
...
| Storybook    | 当前本地构建含 451 个 Story，文档引用 422 个 Story ID 且无失配；81/81 个标题含关键交互或语义断言；历史完整矩阵 444 × 7、共 3,108 个组合通过，新增 Portal/SideNav 已定向复验，0 axe 违规、0 `pageerror` |
| 官网         | 68 个组件页与 8 个公共页面 × Light/Dark、共 152 个场景为最近全量浏览器基线；本批 Next 16 production build 与 82 个静态页面生成通过                                                                     |
| 单元与集成   | 当前工作树 `pnpm check` 全链路通过：测试任务 19/19；mobile 156 个文件、1052/1052 tests；form-react 20 个文件、153/153 tests；Next H5 production build 与移动双引擎 166/166 通过                        |
