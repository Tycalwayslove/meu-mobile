...
| 门禁         | 当前证据                                                                                                                                                          |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 文档映射     | 68 个产品、125 个公开值、383 个公开类型、68 份共置文档；0 未认领、0 覆盖问题                                                                                      |
| Figma        | 54/68 个组件族已建立永久页面链接；剩余 14 项列于 `docs/v2/README.md`，候选冻结前必须补齐                                                                          |
| API          | 226 个结构化类型、2355/2355 个字段有说明、241 个事件；API Extractor 公开契约已同步                                                                                |
| Storybook    | 当前本地构建含 436 个 Story，文档引用 407 个 Story ID 且无失配；80/80 个标题含关键交互或语义断言；完整矩阵历史基线为 374 × 7、共 2,618 个组合，冻结候选后统一刷新 |
| 官网         | 68 个组件页 × Light/Dark、共 136 个场景为本批前的全量基线；本批 Next 16 production build、4/4 条移动双引擎定点场景与 2/2 条 hydration 场景通过                    |
| 单元与集成   | 当前工作树 `pnpm test` 19/19 个任务通过，其中 mobile 为 141 个文件、927/927 tests；Next H5 定点与 hydration 合计 6/6；候选冻结后仍统一重跑完整发布门禁            |
| 兼容静态扫描 | 199 个 production 构建文件通过 Chrome 70 / iOS 13 基线                                                                                                            |
| 体积         | 68/68 产品与 125/125 公开值在预算内；共享 CSS 24,879 B gzip / 32 KiB                                                                                              |
