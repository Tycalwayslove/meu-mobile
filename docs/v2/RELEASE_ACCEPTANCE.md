...
| 门禁         | 当前证据                                                                                                                                                                                   |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 文档映射     | 68 个产品、125 个公开值、372 个公开类型、68 份共置文档；0 未认领、0 覆盖问题                                                                                                               |
| API          | 222 个结构化类型、2290/2290 个字段有说明、237 个事件；API Extractor 公开契约已同步                                                                                                         |
| Storybook    | 历史静态 index 为 394 个 Story；当前源码为 404 个 Story exports、文档 376 项引用（371 个唯一 ID），其中 11 项等待索引刷新；完整矩阵历史基线为 374 × 7、共 2,618 个组合，冻结候选后统一刷新 |
| 官网         | 68 个组件页 × Light/Dark、共 136 个场景为本轮修改前的全量基线；本批 Next 16 production 与定向双引擎 axe 已通过                                                                             |
| 单元与集成   | 本批 Tabs/NavBar/TabBar/List/Cell 共置测试 62/62、Next H5 双引擎 8/8 定向通过；更早全量计数属于未绑定当前 SHA 的历史快照，不作为候选证据，冻结后统一全跑                                   |
| 兼容静态扫描 | 199 个 production 构建文件通过 Chrome 70 / iOS 13 基线                                                                                                                                     |
| 体积         | 68/68 产品与 125/125 公开值在预算内；共享 CSS 23,580 B gzip / 32 KiB                                                                                                                       |
