...
| 门禁         | 当前证据                                                                                                                                                                                                                                           |
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 文档映射     | 68 个产品、125 个公开值、384 个公开类型、68 份共置文档；57/68 已完成本地验证，11 个仍有本地缺口；0 未认领、0 覆盖问题                                                                                                                              |
...
| Storybook    | 当前本地构建含 463 个 Story，文档引用 434 个 Story ID 且无失配；81/81 个标题含关键交互或语义断言；历史完整矩阵 444 × 7、共 3,108 个组合通过，本批四组件 16 × 7、共 112 个直属 Story 组合定向通过，匹配扫描为 140 个组合，0 axe 违规、0 `pageerror` |
| 官网         | 68 个组件页与 8 个公共页面 × Light/Dark、共 152 个场景为最近全量浏览器基线；本批 Next 16 production build 与 82 个静态页面生成通过                                                                                                                 |
| 单元与集成   | 当前工作树 `pnpm check` 全链路通过：测试任务 19/19；mobile 164 个文件、1094/1094 tests；form-react 20 个文件、153/153 tests；Next H5 production build 与移动双引擎 180/180 通过                                                                    |
| 兼容静态扫描 | 200 个 production 构建文件通过 Chrome 70 / iOS 13 基线                                                                                                                                                                                             |
| 体积         | 68/68 产品与 125/125 公开值在预算内；共享 CSS 25,136 B gzip / 32 KiB                                                                                                                                                                               |
