...
| API          | 222 个结构化类型、2276/2276 个字段有说明、237 个事件；API Extractor 公开契约已同步                      |
| Storybook    | 当前目录 379 个 Story；最近一次完整矩阵为 374 × 7，共 2,618 个组合并通过，冻结候选后统一刷新            |
| 官网         | 68 个组件页 × Light/Dark，共 136 个主题、结构、axe 与 React 运行时场景通过                              |
| 单元与集成   | Mobile 652、Form 56、Docs 145；隔离 Next H5 最近一次完整套件 92/92，新场景已完成双引擎定向验证          |
| 兼容静态扫描 | 198 个 production 构建文件通过 Chrome 70 / iOS 13 基线                                                  |
| 体积         | 68/68 产品与 125/125 公开值在预算内；共享 CSS 22,722 B gzip / 32 KiB                                    |
