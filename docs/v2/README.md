...
- 隔离 Next H5 已在移动 Chromium/WebKit 完成 152/152 条全量 E2E，并为初始 open Portal、测量与手势边界建立专项 hydration；输入生命周期 8/8、嵌套 Portal/Popup 生命周期与全直接浮层边界矩阵 8/8、Popover 12-placement/嵌套滚动容器 2/2、ImageViewer RTL/取消恢复 4/4、布局测量/hydration/CLS/系统低动态/RTL 几何/结果焦点专项 16/16；性能/恢复专页 14/14、三轮重复 42/42，覆盖 10,000 行 VirtualList、1,500 节点 TreeSelect、高频/取消手势，以及 Image、InfiniteList、ImageUploader 的确定性失败、重试与取消；`/verification` 的 5 秒工具自检 2/2 通过，可供最终候选执行 60 秒真机采样与导出结构化证据。
