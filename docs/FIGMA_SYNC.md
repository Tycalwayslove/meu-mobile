...
- Batch 5 DatePicker 页面：[DatePicker（230:2）](https://www.figma.com/design/1EjHFQkfyM4FNsfIuhFF1v?node-id=230-2)；组件集：[Meu/DatePicker（231:1047）](https://www.figma.com/design/1EjHFQkfyM4FNsfIuhFF1v?node-id=231-1047)；Light / Dark 预览为 [230:796 / 230:797](https://www.figma.com/design/1EjHFQkfyM4FNsfIuhFF1v?node-id=230-796)
- Batch 5 DatePicker QA：6 个 `Precision × Constraint` variants 覆盖 Day / Minute / Second 与 Default / Bounded 代表场景，分别显示 3 / 5 / 6 列；223/223 个 solid fill 与 12/12 个 stroke 绑定 Foundation 变量，未命名和重复 variant 均为 0。每列固定五行、行高 48px，140 个 option 触控区域全部通过且最小为 94×48px，12 个 header action 目标最小为 84×44px；分钟与秒示例只使用 0–59 内的合法值。DateAdapter、边界、步长、filter、renderLabel、确认式 draft 与表单 adapter 只记录为运行时契约，设计场景轴不伪造公开 API。Code Connect 已按规则仅尝试一次，因当前 Figma 席位与计划限制保持未连接
- Batch 5 TimePicker 页面：[TimePicker（241:2）](https://www.figma.com/design/1EjHFQkfyM4FNsfIuhFF1v?node-id=241-2)；组件集：[Meu/TimePicker（245:2）](https://www.figma.com/design/1EjHFQkfyM4FNsfIuhFF1v?node-id=245-2)；文档与 Light / Dark 预览为 [241:3 / 246:857 / 246:898](https://www.figma.com/design/1EjHFQkfyM4FNsfIuhFF1v?node-id=241-3)
- Batch 5 TimePicker QA：8 个 `Precision × HourCycle × Constraint` variants 覆盖 Minute / Second、24Hour / 12Hour 与 Default / Bounded；8 个 variant 共 108 个真实 option，Light / Dark 预览分别为 10 / 15 个，所有 option 高度均为 48px。12Hour Period 列只包含 AM / PM，不使用伪占位选项；400/400 个 fill 绑定 Foundation 变量，未绑定 fill / stroke、未命名节点、重复 variant 与非法时间值均为 0。8 个 variant 与 2 个 preview 的 header action group 均为 84×44px；canonical 0–23 `TimeValue`、边界、步长、filter、确认式 draft 与表单 adapter 只记录为运行时契约。Code Connect 已按规则仅尝试一次，因当前 Figma 席位与计划限制保持未连接
- Batch 5 Calendar 页面：[Calendar（255:7）](https://www.figma.com/design/1EjHFQkfyM4FNsfIuhFF1v?node-id=255-7)；组件集：[Meu/Calendar（257:722）](https://www.figma.com/design/1EjHFQkfyM4FNsfIuhFF1v?node-id=257-722)；文档、Light / Dark 预览与 QA 为 [255:9 / 258:2 / 258:149 / 258:296](https://www.figma.com/design/1EjHFQkfyM4FNsfIuhFF1v?node-id=255-9)
- Batch 5 Calendar QA：6 个 `Selection × State` variants 覆盖 Single / Multiple / Range 与 Default / Bounded；每个 variant 固定 42 个 72×48px DayCell、42 个 44×44px DayButton 和 2 个 44×44px 导航按钮。Range 的中间日期使用连续 `color/subtle` 色带和 `color/ink` 文本，仅端点使用 accent 圆形按钮；中间文本在 Light / Dark 的对比度分别为 13.29:1 / 11.67:1。362/362 个 solid fill、6/6 个 solid stroke 与 274/274 个相关圆角绑定 Foundation 变量；未绑定 paint、未命名节点、重复 variant、非法/重复日期、文本溢出、重叠和 clipping 候选均为 0。单选、多选、范围完成标记、DateAdapter、键盘与表单即时写入只记录为运行时契约。Code Connect 已按规则仅尝试一次，因当前 Figma 席位与计划限制保持未连接
- Batch 5 DateRangePicker 页面：[DateRangePicker（270:2）](https://www.figma.com/design/1EjHFQkfyM4FNsfIuhFF1v?node-id=270-2)；组件集：[Meu/DateRangePicker（270:676）](https://www.figma.com/design/1EjHFQkfyM4FNsfIuhFF1v?node-id=270-676)；文档、Light / Dark 预览与 QA 为 [270:3 / 270:677 / 270:845 / 270:1013](https://www.figma.com/design/1EjHFQkfyM4FNsfIuhFF1v?node-id=270-3)
- Batch 5 DateRangePicker QA：4 个 `State` variants 覆盖 Empty / Start / Complete / Disabled，摘要保持状态派生内容而非伪造共享属性；333/333 个 fill、24/24 个 stroke 与 28/28 个相关圆角绑定 Foundation 变量，未命名节点、重复 variant、重叠与文本溢出均为 0。4 个 variant 共复用 168 个 44×44px DayButton 和 8 个 44×44px 导航按钮，header action 为 96×48px，preset 为 100×44px；每个 variant 均包含 42 个合法唯一日期，Light / Dark WCAG AA 对比度通过。draft、首次点选不完整、同日二次点选、preset 不自动确认、取消回滚与表单确认写入只记录为运行时契约。Code Connect 未重复尝试，继续保持席位阻塞状态
